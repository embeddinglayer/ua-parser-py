...
REGEXES = {
    ...
    "device": [
        ...
        [r"\b((?:bb[a-f]|st[hv])100-\d)", r"\(bb10; (\w+)"],  # BlackBerry 10
        ...
        [r"\(dtv[\);].+(aquos)", r"(aquos-tv[\w ]+)\)"],  # Sharp
    ...
    "os": [
        ...
        [[VERSION, "_", "."], [NAME, "iOS"]],
        [r"(mac os x) ?([\w\. ]*)", r"(macintosh|mac_powerpc\b)(?!.+haiku)"],  # Mac OS
        ...
        [r"web0s;.+rt(tv)", r"\b(?:hp)?wos(?:browser)?\/([\w\.]+)"],  # WebOS
        [VERSION, [NAME, "webOS"]],
                    ...
                    if isinstance(q, list):
                        if len(q) == 2:
                            ...
                            if callable(q[1]):
                                ...
                                yield q[0], (
                                    re.sub(q[1], q[2].replace("$", "\\"), match)
                                    if match
                                    else None
                                )
                        elif len(q) == 4:
                            yield q[0], (
                                q[3](re.sub(q[1], q[2].replace("$", "\\"), match))
                                if match
                                else None
                            )
                    else:
...
class UAParser:
    ...
    def browser(self) -> Browser:
        if self._browser is None:
            browser = Browser()
            for key, value in rgx_mapper(self.ua, REGEXES["browser"]):
    ...
    def cpu(self) -> CPU:
        if self._cpu is None:
            cpu = CPU()
            for key, value in rgx_mapper(self.ua, REGEXES["cpu"]):
    ...
    def device(self) -> Device:
        if self._device is None:
            device = Device()
            for key, value in rgx_mapper(self.ua, REGEXES["device"]):
    ...
    def engine(self) -> Engine:
        if self._engine is None:
            engine = Engine()
            for key, value in rgx_mapper(self.ua, REGEXES["engine"]):
    ...
    def os(self) -> OS:
        if self._os is None:
            os = OS()
            for key, value in rgx_mapper(self.ua, REGEXES["os"]):
